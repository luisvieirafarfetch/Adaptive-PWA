...
const unsplashKey =
    '4dfb97f5e76ece4280b1b5c474aec5099a5a6564411d94af4fc7f8e4183fe102';
...
app.get('/stories/:id', async (req, res) => {
    ...
    if (useMocks) {
        const mockdata =
            req.params.id === 'new' ? mock.newItems : mock.topItems;
        data = mockdata.slice(0, items);
    } else {
        const stories = id =>
            hnservice.stories(id, {
                count: items,
            });

        const images = useMocks
            ? null
            : axios
                  .get(
                      `https://api.unsplash.com/photos/random?count=${items}&client_id=${unsplashKey}`
                  )
                  .then(response => response.data);

        data = await Promise.all([stories(req.params.id), images]).then(
            response => {
